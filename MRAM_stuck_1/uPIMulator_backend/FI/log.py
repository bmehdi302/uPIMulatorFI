...
def log_injection_run(log_path, injection_id, address, byte, bit_position, result, diff_count):
    """
    Logs each injection run to a file with its parameters and outcome.
    """
    with open(log_path, 'a') as log_file:
        log_file.write(f"Injection {injection_id}, Address: {address}, Byte: {byte}, "
