    ...
    //print mram output
      ...
      for (int i = 0; i < byte_stream->size(); i++) {
          ...
          if (output_byte != golden_byte) {
            std::cout << "Mismatch at index " << i << ": output_byte = "
            << output_byte << ", golden_byte = " << golden_byte << std::endl;
            diff_count++;
          }
      }
      std::cout << "DIFFERENCES: " << diff_count << std::endl;
...
void SchedThread::dma_transfer_dpu_results(int execution) {
  ...
  for (DPUID dpu_id = 0; dpu_id < num_dpus(); dpu_id++) {
    ...
    if (byte_stream != nullptr) {
      ...
      for (int i = 0; i < byte_stream->size(); i++) {
        /*std::cout << std::dec << "wram_byte : " << wram_byte_stream->byte(i)
                    << " ref_byte: " << byte_stream->byte(i) << std::endl;*/
